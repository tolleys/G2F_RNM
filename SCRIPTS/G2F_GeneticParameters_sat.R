...
geno_sub[1:5,1:5]
pheno1 <- merge(pheno, geno_sub, by="PedShort")

head(pheno1)

List_LocYear = unique(pheno1$LocYear)
...
for(i in 1:length(List_LocYear)){
  for(j in 12:33){
    Data13 <- data.frame()
    LY = List_LocYear[i]
    int1 <- subset(pheno1, pheno1$LocYear==LY)
    ...
    m1 = lm(GrainYield_Mg_ha_SpatialCor ~ SNP, int1)
    anova(m1)
...
LD <- read.table("~/Documents/Purdue/PhD/Research/Chp4_RRM_EnvironmentalGradient/RESULTS/LinkageDisequilibrium/G2F_LD_sat.txt", header=T)
str(LD)
...
ggplot(data=LD, aes(x=Dist_bp, y=R.2, colour=as.factor(Locus1))) +
