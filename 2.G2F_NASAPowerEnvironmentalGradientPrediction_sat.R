...
for(i in 2014:2017){
  ...
  print(plot(varImp(g2f_env_pred_train,scale=T),30))
...
ggplot(test1, aes(x=EnvGrad, y=PredEnvGrad)) +
  geom_point(size=2, col="royalblue4") +
  geom_smooth(method=lm, col="black", se = F) +
  annotate("text", x=12.9, y=7.2, size=6, label= "r = 0.38") + 
  theme_bw() +
  theme(axis.title = element_text(size=16, face="bold")) +
  ...
  labs(x=expression(bold(paste("Environmental Merit (Mg h",a^{-1},")"))),
       y=expression(bold(paste("Predicted Environmental Merit (Mg h",a^{-1},")"))))
